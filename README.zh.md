...
6，另外，还可以通过非交互的方式来运行命令，即：通过制定 -cmd参数和-son参数来直接运行某个命令，示例如下：
python ./pycmd.py -cmd=say_hai -son=say -name=约翰 
或者在后台执行
nohup python ./pycmd.py -cmd=say_hai -son=say -name=约翰  > /dev/null 2>&1 &
但是这种执行方式如果参数值包含了空格，那么参数值应该被引号包裹起来
```
...
2024.01.18
1. 修复Bug
    修复了助手命令 assistant 在创建命令时如果 mpth 或者 cdir 参数值的前面包含 . 或则 ./ 时会出现错误的问题
2. README
    在README中添加了使用非交互式方式执行命令的示例
```